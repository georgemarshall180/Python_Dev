...
def insert_result_in_summary_sheet(output_excel, input_sheet, next_available_index_on_summary):
	...
	for row_num in range(1, max_row):
		if row_num == next_available_index_on_summary:
			...
			new_summary_sheet.cell(row=row_num, column=3).value = NULL
			new_summary_sheet.cell(row=row_num, column=4).set_value_explicit( value = "=" + input_sheet.title + "D32", data_type = 'f') 
			...
			new_summary_sheet.cell(row=row_num, column=7).set_value_explicit( value = "=IF(E"+ row_num + "=0,G" +  row_num + ",E"+ row_num + "*(1+G" + row_num + "))", data_type = 'f') 
			new_summary_sheet.cell(row=row_num, column=8).set_value_explicit( value = "=F" + row_num + "-E" +  row_num + ")/E" + row_num, data_type = 'f')
			new_summary_sheet.cell(row=row_num, column=9).value = NULL
			new_summary_sheet.cell(row=row_num, column=10).set_value_explicit( value = "=" + input_sheet.title + "E8", data_type = 'f') 
			...
			if is_new_col_added: # L1 Average & GOAL
				...
				for col_num in range (0, max_col):
					if col_num == 4 or col_num == 7 or col_num == 8 or col_num == 10:
						new_summary_sheet.cell(row = row_num , column = col_num).set_value_explicit(value = old_summary_sheet.cell(row = row_num, column = col_num).value, data_type = 'f')
					else:
						new_summary_sheet.cell(row = row_num , column = col_num).value = old_summary_sheet.cell(row = row_num, column = col_num).value
